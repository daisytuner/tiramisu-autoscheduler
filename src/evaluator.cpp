...
float evaluate_by_execution::evaluate(sdfg_wrapper& sdfg, syntax_tree& ast) const
{
    // Execute and measure
    std::vector<float> measurements = sdfg.benchmark(ast);
    
    std::sort(measurements.begin(), measurements.end());
    double median;
    if (measurements.size() % 2 == 0) {
        size_t n_right = measurements.size() / 2 - 1;
        size_t n_left = n_right - 1;
        median = (measurements[n_left] + measurements[n_right]) / 2.0;
    } else {
        size_t n = measurements.size() / 2;
        median = measurements[n];
    }
...
float evaluate_by_learning_model::evaluate(sdfg_wrapper& sdfg, syntax_tree& ast) const
{
    ...
    try {
        sched_json = get_schedule_json(ast);
    } catch (const std::out_of_range& e) {
        /* This happens when we've generated a schedule that is a valid AST but cannot be understood by the model (e.g., too many swapped dimensions, tiles of tiles of tiles).
        We should change the schedule generator, but hard to debug. */
        return -0.0;
    }
...
std::string evaluate_by_learning_model::get_schedule_json(syntax_tree const& ast)
{
    ...
    for (computation *comp : ast.computations_list)
    {
        ...
        if (interchanged)
        {
            comp_sched_json += "\"" + iterators_list.at(int_l0).name + "\", \"" + iterators_list.at(int_l1).name + "\"";
            
            dnn_iterator dnn_it = iterators_list.at(int_l0);
            iterators_list.at(int_l0) = iterators_list.at(int_l1);
            iterators_list.at(int_l1) = dnn_it;
        }
        ...
        if (tiled)
        {
            if (tile_nb_l == 2)
            {
                ...
                comp_sched_json += "\"" + iterators_list.at(0).name + "\", " + "\"" + iterators_list.at(tile_l0 + 1).name + "\"";
            ...
            } else
            {
                ...
                comp_sched_json += "\"" + iterators_list.at(tile_l0).name + "\", " + "\"" + iterators_list.at(tile_l0 + 1).name + "\", " + "\"" + iterators_list.at(tile_l0 + 2).name + "\"";
        ...
        // JSON for unrolling
        comp_sched_json += "\"unrolling_factor\" : null,";

        // Parallelization tag
        comp_sched_json += "\"parallelized_dim\" : ";
        if (parallelized)
        {
            comp_sched_json += "\"" + iterators_list.at(parallelized_level).name + "\",";
        }
        else
        {
